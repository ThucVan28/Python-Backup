...
# for i in range(0,23):   #67 Put the amount of pictures you have taken for the calibration inbetween range(0,?) wenn starting from the image number 0
#     t= str(i)
#     # ChessImaR= cv2.imread('chessboard-R'+t+'.png',0)    # Right side
#     # ChessImaL= cv2.imread('chessboard-L'+t+'.png',0)    # Left side
#     ChessImaR= cv2.imread('./Calib_cameraR/image_'+t+'.jpg',0)    # Right side
#     ChessImaL= cv2.imread('./Calib_cameraL/image_'+t+'.jpg',0)    # Left side
...
cv2.resizeWindow('img_L', int(width/2), int(height/2))
cv2.imshow('img_L', img_L)


# path_L = "./imag/test/image20.jpg"
# path_R = "./imag/test/image21.jpg"
# img_L = cv2.imread(path_L)
# img_R = cv2.imread(path_R)

# width = int(img_L.shape[1] )
# height = int(img_L.shape[0] )
...
# cv2.namedWindow('img_L', cv2.WINDOW_NORMAL)
# cv2.resizeWindow('img_L', width, height)
# cv2.imshow('img_L', img_L)
...
while True:
    ...
    # frameL = cv2.fastNlMeansDenoisingColored(frameL, None, 10, 10, 7, 15)  
    # frameR = cv2.fastNlMeansDenoisingColored(frameR, None, 10, 10, 7, 15)
